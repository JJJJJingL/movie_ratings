...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 23 20:25:51 2019
...
for one_script in script_list:
    mean = mean_words_per_sentence(one_script)[1]
    if mean < 50:
        mean_list.append(mean)
    else:
        mean_list.append(0)
    number_sent = mean_words_per_sentence(one_script)[2]
    baseline_list.append(number_sent)
script_list_stripped = [script for i, script in enumerate(script_list) if mean_list[i] is not 0]
...
train_X, val_X, train_y, val_y = train_test_split(X_tfidf, y, test_size=0.3)
model = RandomForestRegressor(n_estimators=10, random_state=42)
fitted = model.fit(train_X, train_y)
predicted = model.predict(val_X)
...
# Calculate and display accuracy
accuracy = 100 - np.mean(mape)
print('Baseline with two features accuracy:', round(accuracy, 2), '%.')
